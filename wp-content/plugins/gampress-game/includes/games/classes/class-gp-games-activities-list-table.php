...
if ( !class_exists( 'GP_Games_Activities_List_Table' ) ) :

    class GP_Games_Activities_List_Table extends WP_List_Table {
        ...
        function prepare_items() {
            ...
            $datas = gp_games_get_activities( array(
                'id'               => $id,
        ...
        function get_columns() {
            return array(
                ...
                'create_time'       => __( 'CreateTime', 'gampress-game' ),
                'start_time'        => __( 'StartTime', 'gampress-game' ),
                'status'            => __( 'Status', 'gampress-game' ),
                'expired'           => __( 'Expired', 'gampress-game' )
            );
        }
        ...
        function extra_tablenav( $which ) {
            ...
            <div class="alignleft actions">
                <select name="status">
                    <option value="" <?php selected( !$status ); ?>><?php _e( 'Show all activities', 'gampress' ); ?></option>
        ...
        function column_cb( $item ) {
            printf( '<label class="screen-reader-text" for="activity_id-%1$s">' . __( 'Select activity item %1$s', 'gampress' ) . '</label><input type="checkbox" name="id[]" value="%1$s" id="activity_id-%1$s" />', $item->id );
        }

        function column_name( $item ) {
            echo $item->name;

            $base_url   = gp_get_admin_url( 'admin.php?page=gp-games&amp;id=' . $item->id );
            $edit_url   = add_query_arg( 'action', 'edit', $base_url );
            $actions = array(
