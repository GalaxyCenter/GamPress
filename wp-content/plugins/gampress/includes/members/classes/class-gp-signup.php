...
class GP_Signup {
    ...
    public function populate() {
        ...
        $signups_table = gampress()->members->table_name_signups;
    ...
    public static function get( $args = array() ) {
        ...
        if ( empty( $r['include'] ) ) {
            ...
            if ( ! empty( $r['usersearch'] ) ) {
                $search_terms_like = '%' . gp_esc_like( $r['usersearch'] ) . '%';
            ...
            $sql['order']	= gp_esc_sql_order( $r['order'] );
         ...
         */
        $paged_signups = $wpdb->get_results( apply_filters( 'gp_members_signups_paged_query', join( ' ', $sql ), $sql, $args, $r ) );
         ...
         * @param array  $r     Array of parsed arguments for get() method.
         */
        $total_signups = $wpdb->get_var( apply_filters( 'gp_members_signups_count_query', join( ' ', $sql ), $sql, $args, $r ) );
    ...
    public static function add( $args = array() ) {
        ...
        $r = wp_parse_args( $args,
                ...
                'gp_core_signups_add_args'
                );
        
        $inserted = $wpdb->insert(
                    gampress()->members->table_name_signups,
                    $r,
                    array( '%s', '%s', '%s', '%s', '%s' )
                );
         ...
         */
        return apply_filters( 'gp_core_signups_add', $retval );
    }
    
    /**
     * Create a WP user at signup.
     *
     * Since GP 2.0, non-multisite configurations have stored signups in
    ...
    public static function add_backcompat( $user_login = '', $user_password = '', $user_email = '', $usermeta = array() ) {
         ...
         */
        return apply_filters( 'gp_core_signups_add_backcompat', $user_id );
    }
    ...
    public static function check_user_status( $user_id = 0 ) {
         ...
         */
        return apply_filters( 'gp_core_signups_check_user_status', intval( $user_status ) );
    }
    ...
    public static function validate( $key = '' ) {
         ...
         */
        return apply_filters( 'gp_core_signups_validate', $activated );
    }
    ...
    public static function count_signups() {
         ...
         */
        return apply_filters( 'gp_core_signups_count', (int) $count_signups );
    }
    ...
    public static function update( $args = array() ) {
         ...
         */
        return apply_filters( 'gp_core_signups_update', $r['signup_id'] );
    }
    ...
    public static function resend( $signup_ids = array() ) {
         ...
         */
        do_action( 'gp_core_signup_before_resend', $signup_ids );
        ...
        foreach ( $signups as $signup ) {
            ...
            if ( is_multisite() ) {
                ...
                if ( ! empty( $user_id ) && 2 != self::check_user_status( $user_id ) ) {
                    ...
                    $result['errors'][ $signup->signup_id ] = array( $signup->user_login, esc_html__( 'the sign-up has already been activated.', 'gampress' ) );
                    ...
                    gp_core_signup_send_validation_email( false, $signup->user_email, $signup->activation_key, $signup->user_login );
                }
            }
         ...
         */
        do_action( 'gp_core_signup_after_resend', $signup_ids, $result );
         ...
         * @param array $result Updated metadata related to activation emails.
         */
        return apply_filters( 'gp_core_signup_resend', $result );
    }
    ...
    public static function activate( $signup_ids = array() ) {
         ...
         */
        do_action( 'gp_core_signup_before_activate', $signup_ids );
        
        foreach ( $signups as $signup ) {
            
            $user = gp_core_activate_signup( $signup->activation_key );
         ...
         */
        do_action( 'gp_core_signup_after_activate', $signup_ids, $result );
         ...
         */
        return apply_filters( 'gp_core_signup_activate', $result );
    }
    ...
    public static function delete( $signup_ids = array() ) {
         ...
         */
        do_action( 'gp_core_signup_before_delete', $signup_ids );
        
        foreach ( $signups as $signup ) {
            $user_id = username_exists( $signup->user_login );
            
            if ( ! empty( $user_id ) && $signup->activation_key === gp_get_user_meta( $user_id, 'activation_key', true ) ) {
                ...
                if ( 2 != self::check_user_status( $user_id ) ) {
                    ...
                    gp_core_delete_account( $user_id );
                }
            }
         ...
         */
        do_action( 'gp_core_signup_after_delete', $signup_ids, $result );
         ...
         */
        return apply_filters( 'gp_core_signup_delete', $result );
    }
}
